...
fn validate_codes_alpha(codes:&[(usize, usize)]) -> usize {
    ...
    for &(left, right) in codes {
        for num in left..=right {
            debug_assert!(usize::MAX - num > sum_invalid_codes, "overflow increase integer precision");
    ...
    for &(left, right) in codes {
        ...
        for num in left..=right {
            ...
            while n_len / primes.primes[i] > 0 {
                ...
                i+=1;
                if n_len % pattern_len != 0 { continue; }
...
fn main() {
    ...
    match code_parse {
        Ok(codes) => {
            ...
            println!("Beta Validate Codes {}", validate_codes_beta(&codes, &mut primes));
